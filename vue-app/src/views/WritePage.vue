...
export default {
  props: ['memberId', 'teamId', 'workId'], // 프롭스 정의
  data() {
    return {
      formData: {
        title: '',
    ...
    async submitForm() {
      ...
      try {
        const response = await axios.post(
            `http://localhost:8080/board/multiWrite/${memberId}/${teamId}/${workId}`,
            formData,
            {
              headers: {
